...
| --- | --- | --- | --- |
| OPEA-native depth could be beaten by teams with more official microservices, Helm/GMC, or merged PRs | Official TEI profile is implemented and C3-validated; OPEA component matrix maps Gateway, Megaservice, Dataprep, Retriever/RAG, Embeddings, Vector DB, LLM adapter, Guardrails, and Evaluation; PR-ready package is prepared | `docker-compose.opea-tei.yml`, `docs/official-opea-tei-profile.md`, `docs/opea-native-depth-matrix.md`, `docs/opea-upstream/pr-ready/` | We can claim official OPEA TEI embedding path and GenAIExamples-ready package; do not claim Helm/GMC production deployment or merged upstream PR |
...
| Upstream OPEA PR is not opened/merged | Public RFC and comments are posted; PR-ready package is smoke-tested; minimal first-PR scope is documented for maintainers | `docs/opea-upstream/rfc-issue-draft.md`, `docs/opea-upstream/implementation-feedback-comment.md`, `docs/opea-upstream/tei-update-comment.md`, `docs/opea-upstream/pr-ready/` | If a judge requires an actual upstream PR, this remains weaker than a merged PR; submit the PR manually if fork/write permission becomes available |
...
Gateway/Megaservice, guardrails, a scorecard, C3 Xeon validation, PR-ready OPEA
GenAIExamples material, and a judge-facing Docker/Web/API product.
```
