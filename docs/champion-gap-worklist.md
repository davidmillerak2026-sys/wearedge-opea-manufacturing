...
| Add OPEA runtime wrapper or compose profile | Protects the 20-point "Use of OPEA" subscore | `docker compose up` or `./deploy.sh` starts the Manufacturing demo path |
| Add one-click challenge deployment | Official submission requirement | Fresh clone can run sample Manufacturing flow |
...
| Work item | Reason | Done when |
| --- | --- | --- |
| Add vector DB profile | Submission form includes Vector DB option | Qdrant/Chroma/Milvus/Redis path is runnable and documented |
